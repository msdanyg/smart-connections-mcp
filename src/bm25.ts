...
export class BM25Index {
  ...
  get size(): number { return this.paths.length; }

  /** query에 대해 상위 k개 path 반환 (BM25 score desc, score>0만) */
  topK(query: string, k: number): { path: string; score: number }[] {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (s > 0) scores.push({ path: this.paths[i], score: s });
    }
